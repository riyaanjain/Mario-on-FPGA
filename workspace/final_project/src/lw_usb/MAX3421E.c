...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int check = XSpi_Transfer(&SpiInstance, buffer,NULL,2);
	if(check!=XST_SUCCESS){
		xil_printf("Error 1\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for(unsigned i = 1; i<=nbytes;++i){//To write all bits using 1 buffer
		buffer[i] = data[i-1];
	}

	int check = XSpi_Transfer(&SpiInstance,buffer,NULL,nbytes+1);
	if(check!=0)xil_printf("Error 2\n");
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int check = XSpi_Transfer(&SpiInstance, buff_write, buff_write, 2);

	if(check!=0)xil_printf("Error 3\n");
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE buff_write[nbytes+1];
	buff_write[0] = reg;

	int check = XSpi_Transfer(&SpiInstance, buff_write, buff_write, nbytes+1);
	if(check!=0)xil_printf("Error 4\n");


	for(unsigned j = 1;j<=nbytes;++j){
		data[j-1] = buff_write[j];
	}
